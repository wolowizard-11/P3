...
class Trader:
    def __init__(self):
        ...
        self.position_limits = {
            Product.AMETHYSTS: 50, Product.STARFRUIT: 50, Product.SQUID_INK: 50,
    ...
    def ink_counterparty_orders(self, state: TradingState) -> List[Order]:
        """Round 5 Extension: Reactive Counterparty Flow Tracking for Whale Signals"""
        orders = []
        ...
        # Scan market trades to intercept structural counterparty momentum
        trades = state.market_trades.get(product, [])
        for trade in trades:
            if trade.buyer == 'Olivia':
                best_ask = min(depth.sell_orders.keys())
                max_buy = min(limit - position, -depth.sell_orders[best_ask])
                if max_buy > 0:
                ...
                max_sell = min(limit + position, depth.buy_orders[best_bid])
                if max_sell > 0:
    ...
    def regime_switching_orders(self, state: TradingState, product: str) -> List[Order]:
        """Round 4 Volatility Engine Maintained"""
        orders = []
        if product not in state.order_depths: return orders
        depth = state.order_depths[product]
        if not depth.buy_orders or not depth.sell_orders: return orders
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        """Main orchestrator executing the full Round 5 cumulative universe."""
        result = {}
        ...
        # 2. R1 Legacy Executions
        result[Product.AMETHYSTS] = self.amethyst_orders(state)
        result[Product.STARFRUIT] = self.starfruit_orders(state)
        
        # 3. R5 Counterparty Reactive Engine
        result[Product.SQUID_INK] = self.ink_counterparty_orders(state)
        
        # 4. R2 Basket Arbitrage
